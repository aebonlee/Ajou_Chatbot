...
async def crawl_eng_college_notices(target_date: str):
    base_url = "https://www.ajou.ac.kr/eng/community/notice.do"
    results, seen = [], set()
...
def insert_college_data_to_db(data: list[dict], college: str):
    conn = None
    try:
        conn = psycopg2.connect(host=DB_HOST, database=DB_DATABASE, user=DB_USER, password=DB_PASSWORD, port=DB_PORT)
        cursor = conn.cursor()
        data_to_insert = [(row['id'], row['title'], datetime.strptime(row['date'], '%Y-%m-%d'), row['url'], row['content_text'], json.dumps(row['images']), college) for row in data]
        insert_query = """
        INSERT INTO college_notices (id, title, date, url, content_text, images, college_name)
        VALUES (%s, %s, %s, %s, %s, %s, %s)
        ON CONFLICT (id) DO UPDATE
        SET title = EXCLUDED.title, date = EXCLUDED.date, url = EXCLUDED.url,
            content_text = EXCLUDED.content_text, images = EXCLUDED.images, college_name = EXCLUDED.college_name;
        """
...
def crawl_and_store_eng_college(**kwargs):
    loop = asyncio.get_event_loop()
    target_date = kst_yesterday_str()
    async def run_crawl():
        return await crawl_eng_college_notices(target_date)
    scraped_data = loop.run_until_complete(run_crawl())
    insert_college_data_to_db(scraped_data, '공과대학')

# --- DAG Definition ---
with DAG(
    dag_id="crawl_eng_college_notices_dag",
    start_date=pendulum.datetime(2025, 1, 1, tz="Asia/Seoul"),
    schedule="@daily",
    catchup=False,
    tags=["notice_crawler", "engineering"],
) as dag:
    crawl_task = PythonOperator(
        task_id="crawl_and_store_eng_college_notices",
        python_callable=crawl_and_store_eng_college,
    )